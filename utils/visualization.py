...
def show_lambdas(omegas, class_names, title="", cmap="coolwarm", colors=None):
    ...
    for i in range(n_omegas):
        sns.heatmap(lmbdas[i], ax=axes[i], square=True, cmap=cmap, cbar=i==0, cbar_ax=None if i else cbar_ax, vmin=vmin, vmax=vmax,  center=0)
        axes[i].set_title(class_names[i], fontdict={'color':colors[i][99], 'fontsize':'xx-large','fontweight':'bold'})
        #axes[i].get_xaxis().set_visible(False)
        #axes[i].get_yaxis().set_visible(False)
        axes[i].set_xticklabels(['x', 'y', 'z'], fontsize='large')
        axes[i].set_yticklabels(['x', 'y', 'z'], fontsize='large')
